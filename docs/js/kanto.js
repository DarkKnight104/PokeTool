...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=kantoData[min].Name
          if((min+1)<max)
          {second=kantoData[min+1].Name}
          if((min+1)<max)
          {third=kantoData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == kantoData[min].Type1 || ft == kantoData[min].Type2){
              first=kantoData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/kanto.json")
