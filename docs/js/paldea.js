...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=paldeaData[min].Name
          if((min+1)<max)
          {second=paldeaData[min+1].Name}
          if((min+1)<max)
          {third=paldeaData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == paldeaData[min].Type1 || ft == paldeaData[min].Type2){
              first=paldeaData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/paldea.json")
