...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=hoennData[min].Name
          if((min+1)<max)
          {second=hoennData[min+1].Name}
          if((min+1)<max)
          {third=hoennData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == hoennData[min].Type1 || ft == hoennData[min].Type2){
              first=hoennData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/hoenn.json")
