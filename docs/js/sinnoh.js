...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=sinnohData[min].Name
          if((min+1)<max)
          {second=sinnohData[min+1].Name}
          if((min+1)<max)
          {third=sinnohData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == sinnohData[min].Type1 || ft == sinnohData[min].Type2){
              first=sinnohData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/sinnoh.json")
