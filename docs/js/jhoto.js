...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=jhotoData[min].Name
          if((min+1)<max)
          {second=jhotoData[min+1].Name}
          if((min+1)<max)
          {third=jhotoData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == jhotoData[min].Type1 || ft == jhotoData[min].Type2){
              first=jhotoData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/jhoto.json")
