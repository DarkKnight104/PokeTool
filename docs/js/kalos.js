...
function mkgen(min, max, x, ft="") {
      ...
      while (min<max) {
        ...
        if(ft=="")
        {
          first=kalosData[min].Name
          if((min+1)<max)
          {second=kalosData[min+1].Name}
          if((min+1)<max)
          {third=kalosData[min+2].Name}
          min += 3;
        }else{
          while(min<max){
            if(ft == kalosData[min].Type1 || ft == kalosData[min].Type2){
              first=kalosData[min].Name
              min++
            break;
            }
...
fetch("https://darkknight104.github.io/PokeTool/data/kalos.json")
